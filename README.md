...
> Last updated: July 23, 2026 — Phase: R5 completed; pre-R6 hardening is flashed and automatic checks pass, with focused physical reconfirmation pending
...
The resulting `1.1.0` candidate uses 11,369 / 14,336 bytes of flash (79%) and 321 / 876 bytes of global RAM (36%). Its HEX SHA-256 is `1afb1a6872dfa3bfb4d61f9beb29a84bac7349ccb756595386f597a26743d2b8`. The wire contract, capability bits, and reported firmware version are unchanged.

**Pre-R6 candidate flash evidence recorded on July 23, 2026:** immediately before the write, the guarded development tool verified the exact runtime as firmware `1.1.0`, capabilities `0x007F`, and serial `CK498AED4EBD`. The elevated writer independently reverified the candidate hash and 32,250-byte HEX artifact, required exactly one healthy `4348:55E0` bootloader and no runtime or stock device, then identified CH552 bootloader `2.5.0` with chip ID `8A ED 4E BD`. It wrote all 11,369 bytes, completed full verification, and reported `Reset OK`. The returned runtime passed the same exact identity and `DEVICE_INFO` handshake. A post-flash guarded runtime-to-ROM-to-runtime cycle then reported `BOOTLOADER_READY: 4348:55E0, CH552 ROM 2.50`, completed explicit `ISP_END`, and returned to the exact runtime handshake.

**Result:** R4 remains completed on the physically accepted July 22 image. The July 23 candidate is now flashed and passes its automatic identity, write/verify/reset, handshake, and guarded bootloader-transition checks. Normal USB unplug/replug, watchdog blue indication, all physical inputs, and both physical recovery paths still require focused reconfirmation on this exact candidate before R6.
...
**Post-flash companion evidence recorded on July 23, 2026:** the integrated hardware-test window connected to the exact candidate runtime. Semantic control invocation, without coordinate input, completed `Refresh device info`, `Ping`, the heartbeat-watchdog test, and an explicit reconnect; the final state was `Connected — Exact runtime identity and DEVICE_INFO verified`. Direct all-white and all-off RGB commands both received successful completion. These results verify the live companion command, ACK, watchdog-recovery, and logical reconnect paths, but do not substitute for observing the blue fallback or LED output. No physical controls were operated during the bounded input observation, so the input matrix remains pending rather than failed.
...
Complete the focused physical reconfirmation on the flashed pre-R6 candidate: observe the watchdog blue fallback, verify all four press/release inputs and both encoder directions, perform a normal USB unplug/replug, and repeat both knob-on-connect and runtime-chord recovery paths. If they pass, begin R6 with one operation at a time: first locate, restore, and activate exactly one unambiguous Codex Desktop window through Win32 and UI Automation, then verify the visible postcondition before adding the next physical mapping.
